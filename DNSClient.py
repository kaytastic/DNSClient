...
real_name_server = "8.8.4.4" #Google DNS Server # Research public DNS servers to find a valid DNS server IP address to use
...
def query_local_dns_server(domain,question_type):
    ...
    ip_address = answers[0].to_text() #index of first IP
    return ip_address   
...
def compare_dns_servers(domainList,question_type):
    for domain_name in domainList:
        local_ip_address = local_host_ip(domain_name,question_type)
        public_ip_address = real_name_server(domain_name,question_type)
...
def local_external_DNS_output(question_type):    
    ...
    for domain_name in domainList:
        ip_address = local_host_ip(domain_name,question_type)
